...
const CommunityIssues: React.FC<CommunityIssuesProps> = ({ communityId }) => {
  ...
  const [selectedTechnician, setSelectedTechnician] = useState<Issue['assignedTechnician'] | null>(null);
  ...
  const openPaymentModal = async (technician: Issue['assignedTechnician']) => {
    setSelectedTechnician(technician);
    setShowPaymentModal(true);
    setPaymentAmount('500'); // Default amount
    setQrCodeImage('');
    
    // Generate default QR code with sample amount
    if (technician) {
      await generateQRCode(technician, '500');
    }
  };
  ...
  const generateQRCode = async (technician?: Issue['assignedTechnician'], amount?: string) => {
    const tech = technician || selectedTechnician;
    const amt = amount || paymentAmount;
    ...
    try {
      ...
      const upiId = `technician${tech.id}@paytm`; // Example UPI ID
      const amountValue = parseFloat(amt);
      const name = tech.name;
      const note = `Payment for technician services - ${name}`;
      
      const upiString = `upi://pay?pa=${upiId}&pn=${encodeURIComponent(name)}&am=${amountValue}&cu=INR&tn=${encodeURIComponent(note)}`;
      ...
      // Fallback: generate a simple QR code with text
      try {
        const fallbackQR = await QRCode.toDataURL(`Pay ₹${amt} to ${tech?.name}`, {
          width: 200,
          margin: 2
        });
  ...
  const handleAmountChange = (newAmount: string) => {
    ...
    if (newAmount && parseFloat(newAmount) > 0 && selectedTechnician) {
      generateQRCode(selectedTechnician, newAmount);
    }
  };
  ...
  return (
    <div className="p-4">
              ...
              {issue.assignedTechnician && (
                <div className="mt-4 pt-4 border-t border-gray-100">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {issue.assignedTechnician.phoneNumber && (
                      <button
                        onClick={() => callTechnician(issue)}
                        disabled={callingTechnician === issue.id}
                        className="flex items-center justify-center gap-2 bg-[#FF4500] text-white px-4 py-3 rounded-2xl hover:bg-[#E03E00] transition-colors text-sm font-semibold disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                    ...
                    {/* Pay Technician Button */}
                    <button
                      onClick={() => openPaymentModal(issue.assignedTechnician)}
                      className="flex items-center justify-center gap-2 bg-green-600 text-white px-4 py-3 rounded-2xl hover:bg-green-700 transition-colors text-sm font-semibold"
                    >
                      <CreditCardIcon className="h-4 w-4" />
                      Pay {issue.assignedTechnician.name}
                    </button>
                  </div>
                </div>
              )}
      ...
      {/* Payment Modal */}
      {showPaymentModal && selectedTechnician && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-lg font-bold text-gray-900">
                Pay {selectedTechnician.name}
              </h3>
              <button
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="font-medium text-gray-900">{selectedTechnician.name}</p>
                  <p className="text-sm text-gray-600">Speciality: {selectedTechnician.speciality}</p>
                  <p className="text-sm text-gray-600">Phone: {selectedTechnician.phoneNumber}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  min="1"
                  step="0.01"
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#FF4500] focus:border-transparent transition-colors"
                />
              </div>

              {/* QR Code Display - Always visible when modal is open */}
              <div className="mt-6 text-center">
                <h4 className="text-sm font-medium text-gray-700 mb-4">
                  Scan QR Code to Pay ₹{paymentAmount || '500'}
                </h4>
                <div className="bg-white border-2 border-gray-200 rounded-lg p-4 inline-block">
                  {isGeneratingQR ? (
                    <div className="w-48 h-48 bg-gray-100 border border-gray-300 rounded flex items-center justify-center">
                      <div className="text-center">
                        <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#FF4500] mx-auto mb-2"></div>
                    ...
                    <div className="w-48 h-48 bg-gray-100 border border-gray-300 rounded flex items-center justify-center">
                      <div className="text-center">
                        ...
                        <div className="text-xs text-gray-600">QR Code will appear here</div>
                      </div>
                    </div>
                  )}
                </div>
                <p className="text-xs text-gray-500 mt-3">
                  Payment to: {selectedTechnician.name}
                </p>
